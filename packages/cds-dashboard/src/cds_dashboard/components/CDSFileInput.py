# Just a copy from Solara FileDrop with a different label
import threading
import typing
...
class FileDropZone(FileInput):
    ...
    template_file = (__file__, "CDSFileInput.vue")
...
def CDSFileInput(
    ...
    return _FileInput(label=label, on_total_progress=on_total_progress, on_file=on_file, lazy=lazy, multiple=False)
